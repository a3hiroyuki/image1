...
class PlotPoints():
    def __init__(self):
        ...
        self.checked_point_no = None
        self.checked_points = None
    ...
    def check_distance_points(self, point_no):
        if self.checked_point_no == None:
            self.checked_point_no = point_no
        else:
            checked_first_point = self.points[self.checked_point_no]
            checked_second_point = self.points[point_no]
            self.checked_points = [[checked_first_point.x, checked_second_point.x],[checked_first_point.y, checked_second_point.y]]
            self.checked_point_no = None
...
#         axes.set_ylim(0, 100)
        ...
        axnext = plt.axes([0.5, 0.9, 0.1, 0.075])
    ...
    def update_plot(self):
        ...
        if self.plot_points.checked_points != None:
            x = self.plot_points.checked_points[0]
            y = self.plot_points.checked_points[1]
            self._axes.plot(x, y, markersize=12, markeredgewidth=3, markeredgecolor="blue",markerfacecolor="lightblue")
            a = np.array([x[0], y[0]])
            b = np.array([x[1], y[1]])
            self._axes.text((x[0] + x[1]) / 2, (y[0] + y[1]) / 2 ,np.linalg.norm(a - b),  ha='center', va='bottom')
...
#                 self.plot_points.add_point(event)
            self.plot_points.check_distance_points(point_no)
            self.update_plot()
        # right click
